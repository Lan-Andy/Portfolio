...
DMR.results <- read.csv(file = file.path(DMR.input, "DMR_results_asthma_vs_control_750gap.csv"),
               header = TRUE, row.names = 1)
DMR.ident <- read.csv(file = file.path(DMR.input, "DMR_identification_AsthmaVsHealthy.csv"),
                        header = TRUE, row.names = 1)
meth.data <- read.csv(file.path(DMR.input, "DMR_all_CpGs_betas.csv"),
                      header = TRUE, row.names = 1)
DMR.data <- read.csv(file.path(input.dir, "Data/006 Differential Methylated Region Analysis DMR/DMR_mval_AsthmaVsHealthy_allSamples.csv"),
                     header = TRUE, row.names = 1)
...
top.DMR <- "chr1:113887281-113887658"           
top.DMR <- DMR.results$name

top.DMR <- "chr1:156875119-156876544"
top.DMR <- "chr1:156862644-156864002"
top.DMR <- c("chr1:156862644-156864002", "chr1:156875119-156876544")

top.DMR <- "chr1:24964550-24966247"
...
################################################################################
## Not needed the execute for individual plots, no idea why tho
...
# plot.data <- plot.data[na.omit(match(identification$CpG, plot.data$CpG)), ]
...
################################################################################
################################################################################
### No need to execute this when making individual CpGs plot and DMR plot
### Trying to find DMRs specific for the group of interest
...
tiff(file.path("C:/Users/24976197/OneDrive - UTS/Desktop/PhD UTS Data folder/004 Bioinformatics/ATLANTIS",
               "/Results/006 Differential Methylated Region Analysis DMR/topDMR_sig_CpGs_AsthmaHealthy_FINAL.tiff"), width = 3000, height = 3000, res = 300)
print(p_all)
dev.off()

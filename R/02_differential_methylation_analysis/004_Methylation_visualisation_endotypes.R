...
meth.data <- read.csv(file = file.path(base.dir,"Data/002 Differential Methylation Analysis DMA/Correct for PC1-3 - Th2 groups/sig_Th2HighVsHealthy_ATLANTIS_Mvalues_dasen.csv"),
                      header = TRUE,
                      row.names = 1)
...
DMA.result <- read.csv(file = file.path(base.dir, "Data/002 Differential Methylation Analysis DMA/Correct for PC1-3 - Th2 groups/DMA_Tt2_significant_CpG_Th2HighVsHealthy_BH.csv"),
                       header = TRUE,
                       row.names = 1)
# DMA.result <- read.csv(file = file.path(base.dir, "Data/002 Differential Methylation Analysis DMA/Correct for PC1-3 - Th2 groups/DMA_Tt2_significant_CpG_Th2HighvsTh2Low_BH.csv"),
#                        header = TRUE,
...
incl.endotypes = TRUE
plot.data <- as.data.frame(t(meth.data[, covariates$meth_file_id]))
if (incl.endotypes){
  plot.data$status <- covariates$group_th
  plot.data$status <- factor(plot.data$status, levels = c("healthy", "low", "undeterm/cor_bio", "high"))
  ...
  plot.data$status <- gsub(pattern = "undeterm/cor_bio", replacement = "Th2-intermediate", x = plot.data$status)
...
dmr_columns <- colnames(plot.data)[colnames(plot.data) != "status"]
...
for (dmr in dmr_columns) {
  ...
  group_means <- aggregate(
    plot.data[[dmr]] ~ plot.data$status,
    data = plot.data,
    FUN = mean
  )
...
for (dmr in unique(result$DMR)) {
  ...
  p_value <- DMA.result[rownames(DMA.result) == dmr, "P.Value"]
  adj_p_value <- DMA.result[rownames(DMA.result) == dmr, "adj.P.Val"]
  ...
  # Calculate the desired differences using Th2-High as the center point
  th2_high_minus_healthy <- mean_th2_high - mean_healthy
  th2_high_minus_th2_low <- mean_th2_high - mean_th2_low
  
  # Calculate the overall difference
  difference <- th2_high_minus_healthy - th2_high_minus_th2_low
  
  # Append the results to the new dataframe
  difference_results <- rbind(difference_results, data.frame(
    DMR = dmr,
    Th2_High_minus_Healthy = th2_high_minus_healthy,
    Th2_High_minus_Th2_Low = th2_high_minus_th2_low,
    Difference = difference,
    p.value = p_value,
    adjust.pvalue = adj_p_value
  ))
}
difference_results <- difference_results[order(abs(difference_results$Difference)), ]
filtered_results <- difference_results[difference_results$adjust.pvalue <= 10E-5, ]
filtered_results <- filtered_results[abs(filtered_results$Th2_High_minus_Healthy) >= 0.2 
                                     & abs(filtered_results$Difference) <= 0.02
                                     , ]

top.cpgs <- filtered_results$DMR
top.cpgs <- "cg06558622"
top.cpgs <- "cg25087851"
...
tiff(file.path("C:/Users/24976197/OneDrive - UTS/Desktop/PhD UTS Data folder/004 Bioinformatics/ATLANTIS",
               "/Results/002 Differential Methylation Analysis DMA/topCpG_Th2_high.tiff"), width = 1000, height = 1000, res = 300)
for (cpg in top.cpgs) {
